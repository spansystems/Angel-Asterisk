		...
		if (event.getSource().getClass()
			...
			if (event.getSource() == server.getUserChannel().getChannel()) {
				...
				if (channel.getState() == ChannelState.UP) {
					...
					server.getUserChannel().setState(State.ESTABLISHED);
					System.out.println("Status of the system is established");
					System.out.println("The agent is now talking to user "+server.getUserChannel().getChannel().getName());										
					server.setCurrentState(new EstablishedState());	
					//server.changeState(new SuperVisorSpyingState());
					//server.channelSpy();											
